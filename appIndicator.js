...
class AppIndicatorsIconActor extends St.Icon {
    ...
    _init(indicator, iconSize) {
        ...
        if (GObject.signal_lookup('resource-scale-changed', this))
            this.connect('resource-scale-changed', () => this._invalidateIconWhenFullyReady());
        else
            this.connect('notify::resource-scale', () => this._invalidateIconWhenFullyReady());

        Util.connectSmart(themeContext, 'notify::scale-factor', this, () => {
            this._updateIconSize();
            this._constrainIconGeometry();
            this._invalidateIconWhenFullyReady();
        });
    ...
    async _createIconByIconData(iconData, iconSize, iconScaling, cancellable) {
        ...
        try {
            ...
            if (width >= height * 1.5) {
                ...
                await this._loadCustomImage(file, iconSize, iconScaling, cancellable);
                return null;
    ...
    get _effectiveIconSize() {
        return this._forcedIconSize || this._iconSize;
    }

    // Whoever shows the icon outside of the panel has to stay the source of
    // truth for its size: an icon that finishes loading later would otherwise
    // restore the panel size behind its back.
    setForcedIconSize(iconSize) {
        if (this._forcedIconSize === iconSize)
            return;
    ...
    _iconPixelSize() {
        ...
        return this._effectiveIconSize * scaleFactor;
    }

    _constrainIconGeometry() {
        const size = this._iconPixelSize();
        this.set({
            width: size,
            height: size,
    ...
    // The image is always shown at the icon size, so that a tray item never
    // becomes wider than its neighbours.
    _setImageContent(content) {
        this._constrainIconGeometry();
        this.set({
            content,
    ...
    _updateIconSize() {
        ...
        this.set_style(`${AppIndicatorsIconActor.DEFAULT_STYLE}; ` +
            `icon-size: ${this._effectiveIconSize}px`);
        this.set_icon_size(this._effectiveIconSize);
        this._constrainIconGeometry();
    }
