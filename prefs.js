...
class AppIndicatorPreferences extends Gtk.Box {
    _init() {
        ...
        this.overflow_hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL,
            spacing: 10,
            margin_start: 10,
            margin_end: 10,
            margin_top: 10,
            margin_bottom: 10 });
        label = new Gtk.Label({
            label: _('Enable tray overflow (Windows-style)'),
            hexpand: true,
            halign: Gtk.Align.START,
        });
        widget = new Gtk.Switch({ halign: Gtk.Align.END });
        this._settings.bind('overflow-enabled', widget, 'active',
            Gio.SettingsBindFlags.DEFAULT);
        if (imports.gi.versions.Gtk === '4.0') {
            this.overflow_hbox.append(label);
            this.overflow_hbox.append(widget);
        } else {
            this.overflow_hbox.pack_start(label, true, true, 0);
            this.overflow_hbox.pack_start(widget, false, false, 0);
        }

        this.overflow_hide_new_hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL,
            spacing: 10,
            margin_start: 10,
            margin_end: 10,
            margin_top: 10,
            margin_bottom: 10 });
        label = new Gtk.Label({
            label: _('Hide new tray icons until pinned'),
            hexpand: true,
            halign: Gtk.Align.START,
        });
        widget = new Gtk.Switch({ halign: Gtk.Align.END });
        this._settings.bind('overflow-hide-new', widget, 'active',
            Gio.SettingsBindFlags.DEFAULT);
        if (imports.gi.versions.Gtk === '4.0') {
            this.overflow_hide_new_hbox.append(label);
            this.overflow_hide_new_hbox.append(widget);
        } else {
            this.overflow_hide_new_hbox.pack_start(label, true, true, 0);
            this.overflow_hide_new_hbox.pack_start(widget, false, false, 0);
        }

        this.overflow_side_hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL,
            spacing: 10,
            margin_start: 10,
            margin_end: 10,
            margin_top: 10,
            margin_bottom: 10 });
        label = new Gtk.Label({
            label: _('Overflow entry position'),
            hexpand: true,
            halign: Gtk.Align.START,
        });
        widget = new Gtk.ComboBoxText();
        widget.append('start', _('Before the tray icons'));
        widget.append('end', _('After the tray icons'));
        this._settings.bind('overflow-button-side', widget, 'active-id',
            Gio.SettingsBindFlags.DEFAULT);
        if (imports.gi.versions.Gtk === '4.0') {
            this.overflow_side_hbox.append(label);
            this.overflow_side_hbox.append(widget);
        } else {
            this.overflow_side_hbox.pack_start(label, true, true, 0);
        ...
        // Icon spacing (horizontal padding on each side of a tray icon)
        this.icon_spacing_hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL,
            ...
            margin_bottom: 10 });
        label = new Gtk.Label({
            label: _('Icon spacing (min: 0, max: 24)'),
            hexpand: true,
            halign: Gtk.Align.START,
        });
        widget = new Gtk.SpinButton({ halign: Gtk.Align.END });
        ...
        if (imports.gi.versions.Gtk === '4.0') {
            this.icon_spacing_hbox.append(label);
            this.icon_spacing_hbox.append(widget);
        } else {
            this.icon_spacing_hbox.pack_start(label, true, true, 0);
            this.icon_spacing_hbox.pack_start(widget, false, false, 0);
        }
